...
same first-write readiness gate before opening `/dev/c30d`. First-write/tiny-pulse
readiness uses `--c30d-warmup-duration 1.0` by default to read and discard startup C30D
feedback before the counted validation window. For real pulses, `--readiness-retries`
...
to `0.05` and duration may be at most `0.50` seconds. The safe zero/stop frame is
...
`pulse_frame_hex`, `allow_extended_low_speed_stream`, stream mode/rate/durations, frames written by phase, baseline vs
