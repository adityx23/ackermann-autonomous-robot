...
- Hard limits reject `abs(target_x) > 0.05`, duration above `0.15` seconds, and stream rates above `50` Hz. Only when both `--stream-mode` and `--allow-extended-low-speed-stream` are set may duration exceed `0.15`, and then it is capped at `0.50` seconds with the same `abs(target_x) <= 0.05` limit.
