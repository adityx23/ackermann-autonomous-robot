...
from ackermann_robot.drivers.c30d_ros_command_frame import (
    CHECKSUM_INDEX,
    build_ackermann_ros_command_frame,
    build_ackermann_ros_command_frame_from_floats,
    build_ros_command_frame,
...
def test_ros_command_frame_shape_and_checksum():
    frame = build_ros_command_frame(0x00, 0x01, 1000, 0, -250)
...
def test_ros_command_frame_signed_int16_big_endian_positive_and_negative():
    frame = build_ros_command_frame(0x00, 0x00, 1000, -1000, -1)
...
def test_ackermann_helper_keeps_y_zero_by_default():
    frame = build_ackermann_ros_command_frame(0x00, 0x00, target_x=123, target_z=-456)
...
def test_build_ros_command_frame_script_prints_disabled_status(capsys):
    module = load_script("build_c30d_ros_command_frame.py")
